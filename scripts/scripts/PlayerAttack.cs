...
public class PlayerAttack : MonoBehaviour
{
    ...
    IEnumerator Attack(int n)
    {
        ...
        Ani.SetBool("attackLeft", false);
        if (gameObject.name == "Boss") {
            rb.velocity = new Vector2( transform.position.x- Player.transform.position.x, transform.position.y - Player.transform.position.y).normalized*Time.deltaTime*400;
            yield return new WaitForSeconds(0.5f);
        }
        attacking1 = false;
